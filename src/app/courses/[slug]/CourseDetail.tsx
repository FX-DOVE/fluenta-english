...
import { courseCopy, courseCategoryLabel, courseLevelLabel } from "@/lib/courseI18n";
...
export function CourseDetail({ course }: { course: Course }) {
  ...
  function badgeText() {
    if (course.badge === "Bestseller") return t("bestseller");
    if (course.badge === "Nuevo") return t("nuevo");
    if (course.badge === "Popular") return t("popular");
    return null;
  }
      ...
      <div className="grid gap-8 lg:grid-cols-[1.4fr_0.9fr]">
        <div>
          <div className="flex flex-wrap gap-2 text-xs font-semibold">
            ...
            <span className="rounded-full bg-sand-100 px-3 py-1 text-ink-800">
              {courseLevelLabel(course, lang)}
            </span>
            <span className="rounded-full bg-sand-100 px-3 py-1 text-ink-800">
              {courseCategoryLabel(course, lang)}
            </span>
            {badge && (
              <span className="rounded-full bg-teal-100 px-3 py-1 text-teal-800">{badge}</span>
            )}
          </div>
          <h1 className="mt-4 font-display text-4xl font-semibold text-ink-900">{c.title}</h1>
          <p className="mt-3 max-w-2xl text-slate-600">{c.description}</p>
      ...
      <div className="mt-12 grid gap-8 lg:grid-cols-3">
              ...
              <li key={l.slug}>
                ...
                >
                  <div>
                    ...
                    {l.quiz && <p className="text-xs text-brand-600">{t("includes_quiz")}</p>}
                  </div>
        ...
        <aside>
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-card">
            <h3 className="font-bold text-ink-900">{t("outcomes")}</h3>
